                            ...
                            {cart.map(item => (
                                <div key={item.id} className="flex items-center space-x-3 space-x-reverse">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-gray-400 font-poppins">${item.price.toFixed(2)}</p>
                                        <div className="flex items-center bg-white/10 rounded-full mt-1 w-fit">
                                            <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1 hover:text-amber-400"><PlusIcon className="w-3 h-3" /></button>
                                            <span className="px-2 text-sm font-bold font-poppins">{item.quantity}</span>
                                            <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 hover:text-amber-400"><MinusIcon className="w-3 h-3" /></button>
                                        </div>
                                    </div>
                                    <button onClick={() => removeFromCart(item.id)} className="p-1 text-gray-500 hover:text-red-500"><TrashIcon className="w-5 h-5" /></button>
                                </div>
                            ))}
                        </div>
                    ) : (
