...
const ReptileCard: React.FC<ReptileCardProps> = ({ reptile, index = 0, variant = 'grid', setPage }) => {
    ...
    const navigateToDetails = () => setPage && setPage(`product/${reptile.id}`);

    const handleWishlistToggle = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (!user) { alert('يُرجى تسجيل الدخول أولاً للمتابعة'); return; }
            ...
            >
                ...
                <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 relative z-10 w-full sm:w-auto">
                    ...
                    <button
                        ...
                        className="p-3.5 sm:p-4 bg-amber-500 text-gray-900 rounded-2xl hover:bg-amber-400 transition-all shadow-xl active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
