        ...
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
          <a href="#" onClick={(e) => handleLinkClick(e, 'home')} className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-xl font-black text-amber-400 leading-none">بيت</span>
              <span className="text-sm font-bold text-white leading-none mt-1">الزواحف</span>
            </div>
          </a>
          ...
          <div className="flex items-center gap-4">
            <button onClick={(e) => handleLinkClick(e, 'cart')} className="relative p-2 text-gray-300 hover:text-white" aria-label="السلة">
              <ShoppingCartIcon className="w-5 h-5" />
              {cart.length > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-amber-500 text-gray-900 text-[10px] font-black rounded-full flex items-center justify-center">
                  {cart.length}
                </span>
              )}
            </button>
            {user ? (
              <div className="relative" ref={userMenuRef}>
                <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="p-1.5 bg-white/5 border border-white/10 rounded-xl flex items-center gap-2" aria-label="قائمة المستخدم">
                  <div className="w-7 h-7 bg-amber-500 text-gray-900 rounded-lg flex items-center justify-center font-black text-xs">{user.name.charAt(0)}</div>
              ...
              <button onClick={(e) => handleLinkClick(e, 'login')} className="bg-amber-500 text-gray-900 px-6 py-2 rounded-xl font-black text-sm">تسجيل الدخول</button>
            )}
          </div>
        </div>
      </nav>
