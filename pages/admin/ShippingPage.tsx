                    ...
                    <div key={order.id} className="glass-dark border border-white/10 rounded-[2rem] p-8 flex flex-col md:flex-row items-center gap-8 group hover:border-amber-500/30 transition-all shadow-xl">
                        <div className="flex -space-x-4 space-x-reverse">
                            {order.items.map((item, i) => (
                                <img key={i} src={item.imageUrl} className="w-16 h-16 rounded-2xl border-4 border-[#0a0c10] object-cover shadow-2xl" />
                            ))}
                        </div>
                        ...
                        <div className="w-full md:w-64">
                            <label className="text-[10px] text-amber-500/60 font-black uppercase tracking-widest mb-2 block">تحديث حالة التوصيل</label>
                            <select 
                                value={order.status}
