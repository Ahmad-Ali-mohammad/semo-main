                        ...
                        {cart.map(item => (
                            <div key={item.id} className="flex items-center space-x-4 space-x-reverse border-b border-white/10 pb-4 last:border-b-0">
                                ...
                                <div className="flex items-center bg-white/10 rounded-full">
                                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-2 hover:text-amber-400"><PlusIcon className="w-4 h-4" /></button>
                                    <span className="px-3 font-bold font-poppins">{item.quantity}</span>
                                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-2 hover:text-amber-400"><MinusIcon className="w-4 h-4" /></button>
                                </div>
